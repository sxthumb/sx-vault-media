...
async fn test_stream_pipe_as_stream_operator() {
    let op1 = FnOperator::new("op1").extract_it(|_chunk, _state, _ctx, _emitter| Ok(()), |_state, _ctx, _emitter| Ok(()));
    let op2 = FnOperator::new("op2").extract_it(|_chunk, _state, _ctx, _emitter| Ok(()), |_state, _ctx, _emitter| Ok(()));
...
async fn test_reactive_stream_pipe_execution() {
    ...
    let op = FnOperator::new("passthrough").extract_it(|_chunk, _state, _ctx, _emitter| Ok(()), |_state, _ctx, _emitter| Ok(()));
